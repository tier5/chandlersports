...
if (post_password_required()) { 
	art_post_box('', '<p class="nocomments">' . __('This post is password protected. Enter the password to view comments.', 'kubrick') . '</p>');
	return;
}
...
if (have_comments()){
    ...
    art_page_navi('<span id="comments">' . ob_get_clean() . ' ' . sprintf(__('to &#8220;%s&#8221;', 'kubrick'), the_title('', '', false)) . '</span>', true);
    echo '<ul class="commentlist">';
    wp_list_comments('type=all&callback=art_comment');
    echo '</ul>';
    art_page_navi('', true);
} elseif('open' != $post->comment_status && !is_page()) { 
    art_post_box('', '<p class="nocomments">' . __('', 'kubrick') .'</p>');
}
...
<?php else : ?>
<p><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" /><label for="author"><small><?php _e('Name', 'kubrick'); ?> <?php if ($req) _e("(required)", "kubrick"); ?></small></label></p>
