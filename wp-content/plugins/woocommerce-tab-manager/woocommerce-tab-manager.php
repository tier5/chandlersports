...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.4.0', __( 'WooCommerce Tab Manager', 'woocommerce-tab-manager' ), __FILE__, 'init_woocommerce_tab_manager', array(
...
class WC_Tab_Manager extends SV_WC_Plugin {
	 ...
	 *
	 * @TODO Remove this as part of WC 2.7 compat {JS 2016-05-24}
	 *
	 * @since 1.6.0
	 */
	public function __get( $name ) {
		...
		switch ( $name ) {
			...
			case 'search':
				...
				_deprecated_function( 'wc_tab_manager()->search', '1.6.0', 'wc_tab_manager()->get_search_instance()' );
				return $this->get_search_instance();

			case 'wc_settings':

				/* @deprecated since 1.6.0 */
				_deprecated_function( 'wc_tab_manager()->wc_settings', '1.6.0', 'wc_tab_manager()->get_settings_instance()' );
				return $this->get_settings_instance();

			case 'ajax':

				/* @deprecated since 1.6.0 */
				_deprecated_function( 'wc_tab_manager()->ajax', '1.6.0', 'wc_tab_manager()->get_ajax_instance()' );
	...
	public function add_plugin_action_links( $actions ) {
		...
		$custom_actions = array(
			...
			'docs'      => sprintf( '<a href="%1$s">%2$s</a>', 'http://docs.woothemes.com/document/tab-manager/', __( 'Docs', 'woocommerce-tab-manager' ) ),
			'support'   => sprintf( '<a href="%1$s">%2$s</a>', 'http://support.woothemes.com/', __( 'Support', 'woocommerce-tab-manager' ) ),
			...
			foreach ( $this->product_tabs[ $product_id ] as $id => $tab ) {
				...
				if ( $tab['id'] == $tab_id ) {
					...
					if ( $get_the_content && ! isset( $tab['content'] ) ) {
						...
						$tab_id   = (int) apply_filters( 'wc_tab_manager_tab_id', $tab_id );
	...
	public function batch_update_products( array $args = array() ) {
		...
		$products_count = $offset + count( $products );
		...
		foreach ( $products as $product ) {
			...
			if ( ! empty( $tabs ) ) {
				...
				$this->get_search_instance()->update_relevanssi_index_for_product( $product->ID );
			}
		}
	 ...
	 * @global object $current_screen The current admin screen object.
	 *
	 ...
	 *
	 * @since 1.4.0
	 * @param  mixed $_post A WP_Post instance, numeric post ID, object, or array. If an object or array is passed it should match the structure of a WP_Post and contain all of the same data (or at least the data that you need returned).
	 * @return WP_Post|bool WP_Post if the post exists or false if not.
	 */
	public function ensure_post( $_post = null ) {

		// If no post data was passed return the current global instance.
		if ( empty( $_post ) && $GLOBALS['post'] ) {
			return $GLOBALS['post'];
		}

		// If a valid WP_Post instance was passed just return it.
		$is_wp_post  = ( $_post instanceof WP_Post );
		if ( $is_wp_post && isset( $_post->ID ) ) {
			return $_post;
		}

		// Bail if the post data isn't an ID, object, or array.
		$is_post_id     = is_numeric( $_post );
		$is_post_object = ( is_object( $_post ) && isset( $_post->ID ) );
		$is_post_array  = ( is_array( $_post ) && isset( $_post['ID'] ) );
		...
		if ( $is_post_array ) {
			$_post = $this->array_to_object( $_post );
		}
