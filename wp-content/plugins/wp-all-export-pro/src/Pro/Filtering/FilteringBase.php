...
abstract class FilteringBase implements FilteringInterface
{
    ...
    protected function isExportNewStuff(){
        return ( ! empty(\XmlExportEngine::$exportOptions['export_only_new_stuff']) and ! empty($this->exportId) );
    }
    ...
    protected function parse_date_field(&$rule ){
        ...
        $rule->value = strpos($rule->value, ":") !== false ? date("Y-m-d H:i:s", strtotime($rule->value)) : date("Y-m-d", strtotime($rule->value));
