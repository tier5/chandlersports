            ...
            default:
                if (strpos($rule->element, "cf_") === 0)
                {
                    ...
                    if ($rule->condition == 'is_empty'){
                        $this->queryJoin[] = " LEFT JOIN {$this->wpdb->usermeta} ON ({$this->wpdb->usermeta}.user_id = {$this->wpdb->users}.ID AND {$this->wpdb->usermeta}.meta_key = '$meta_key') ";
                        $this->queryWhere .= "{$this->wpdb->usermeta}.umeta_id " . $this->parse_condition($rule);
                    }
                    else{
                        $this->queryJoin[] = " INNER JOIN {$this->wpdb->usermeta} ON ({$this->wpdb->usermeta}.user_id = {$this->wpdb->users}.ID) ";
                        $this->queryWhere .= "{$this->wpdb->usermeta}.meta_key = '$meta_key' AND {$this->wpdb->usermeta}.meta_value " . $this->parse_condition($rule);
                    }
                }
                break;
        }
