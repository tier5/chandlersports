...
( function( $, _ ) {
    ...
    _.extend( media.view.Selection.prototype, {
        ...
        clear: function( event ) {
            ...
            var spinner = this.controller.content.get().toolbar.get('spinner');
        ...
        deleteSelected: function( event ) {
            ...
            var data = {},
                ...
                content = this.controller.content.get(),
                spinner = content.toolbar.get('spinner'),
                sidebar = content.sidebar.get( 'bulk-edit' ),
                $errorMessage = this.controller.toolbar.get().primary.$el.find('#eml-bulk-save-changes-failure');
            ...
            spinner.show();
            sidebar.$el.find('input').prop('disabled', true);

            selection.bulkDelete( data ).always( function() {

                spinner.hide();
                sidebar.$el.find('input').prop('disabled', false);
                ...
                _.each( taxonomies, function( terms, taxonomy ) {
                    ...
                    _.each( terms, function( term_id, id ) {
                        ...
                        if ( _.indexOf( terms2remove, 'tax_input['+taxonomy+']['+term_id+']' ) > -1 ) {
                            taxonomies[taxonomy].splice( id, 1 );
                            changed = true;
                        }
                        else {
                            tdata[ 'attachments['+attachment.id+']['+taxonomy+']['+term_id+']' ] = term_id;
                        }
                    });
                    ...
                    _.each( terms2add, function( value ) {
                        ...
                        if ( tax && t_id && tax == taxonomy && _.indexOf( taxonomies[taxonomy], t_id ) == -1 ) {
                            ...
                            taxonomies[taxonomy].push( t_id );
                            changed = true;

                            tdata[ 'attachments['+attachment.id+']['+taxonomy+']['+t_id+']' ] = t_id;
                        }
                    });
                    ...
                    if ( changed ) {
                        ...
                        if ( _.isEmpty( tdata ) ) {
                            tdata[ 'attachments['+attachment.id+']['+taxonomy+']' ] = '';
                        }
