		...
		foreach ( $item as $key => $value ) {
			...
			elseif ( strstr( $key, 'tax:' ) ) {
				...
				if ( $taxonomy == 'product_type' ) {
					...
					if ( is_array( $terms ) ) $terms = $terms['name'];

					if ( ! in_array( $terms, $this->allowed_product_types ) ) {
						$WC_CSV_Product_Import->log->add( sprintf( __('> > > Product type "%s" not allowed - using simple.', 'wc_csv_import'), $terms ) );
						$terms = 'simple';
					}
				}
			...
			elseif ( strstr( $key, 'attribute:' ) ) {
				...
				$attribute_name = trim( str_replace( 'attribute:', '', $key ) );
				...
				if ( substr( $attribute_name, 0, 3 ) == 'pa_' ) {
					...
					if ( ! $value || ! $attribute_name ) continue;
			...
			elseif ( strstr( $key, 'attribute_data:' ) ) {
				...
				$attribute_name = trim( str_replace( 'attribute_data:', '', $key ) );
				...
				if ( ! isset( $attributes[$attribute_name] ) ) $attributes[$attribute_name] = array();

				$attributes[$attribute_name]['position']		= $position;
				$attributes[$attribute_name]['is_visible']		= $visible;
				$attributes[$attribute_name]['is_variation']	= $variation;
			}
			...
			elseif ( strstr( $key, 'attribute_default:' ) ) {
				...
				$attribute_name = trim( str_replace( 'attribute_default:', '', $key ) );
