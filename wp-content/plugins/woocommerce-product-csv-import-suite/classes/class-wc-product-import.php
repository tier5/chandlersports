				...
				<td width="25%">
					<?php
						if ( strstr( $key, 'tax:' ) ) {
							...
							printf(__('Product Attribute: <strong>%s</strong>', 'wc_csv_import'), $column);

						} elseif ( strstr( $key, 'attribute_data:' ) ) {

							$column = trim( str_replace( 'attribute_data:', '', $key ) );
							printf(__('Product Attribute Data: <strong>%s</strong>', 'wc_csv_import'), $column);

						} elseif ( strstr( $key, 'attribute_default:' ) ) {

							$column = trim( str_replace( 'attribute_default:', '', $key ) );
							printf(__('Product Attribute default value: <strong>%s</strong>', 'wc_csv_import'), $column);
...
<?php
	...
	function import() {
		...
		echo '<div class="updated settings-error below-h2"><p>
			'.sprintf(__('Import complete - imported <strong>%s</strong>, merged <strong>%s</strong>, skipped <strong>%s</strong>', 'wc_csv_import'), $this->imported, $this->merged, $this->skipped ).'
		</p></div>';
	...
	function product_exists( $title, $content = '', $date = '' ) {
		global $wpdb;
	    ...
	    $query = "SELECT ID FROM $wpdb->posts WHERE post_type = 'product' AND post_status IN ( 'publish', 'private', 'draft' )";
	    $args = array();
	...
	function process_product( $post ) {
		...
		if ( ! $merging ) {
			$post_exists = $this->product_exists( $post['post_title'], $post['post_content'], '' );
			if ( $post_exists ) {

				// Check unique SKU
				$post_exists_sku = get_post_meta( $post_exists, '_sku', true );

				if ( $post['sku'] == $post_exists_sku ) {
					$this->skipped++;
		...
		if ( ! empty( $post['terms'] ) && is_array($post['terms']) ) {
			...
			foreach ( $post['terms'] as $term_group ) {
				...
				foreach ( $terms as $term ) {
					...
					if ( $term_parent ) {
						$parent_term_exists = term_exists( $term_parent, $taxonomy );
						$parent_term_id 	= is_array( $parent_term_exists ) ? $parent_term_exists['term_id'] : 0;
						if ( $parent_term_id ) $term_args['parent'] = $parent_term_id;
					}

					$term_exists 	= term_exists( $term_name, $taxonomy, $parent_term_id );
					$term_id 		= is_array( $term_exists ) ? $term_exists['term_id'] : 0;

					if ( ! $term_id ) {
						$t = wp_insert_term( $term_name, $taxonomy, $term_args );

						if ( ! is_wp_error( $t ) ) {
							$term_id = $t['term_id'];
						} else {
							$this->log->add( sprintf( __( '> > Failed to import term %s %s', 'wc_csv_import' ), esc_html($term_name), esc_html($taxonomy) ) );
							continue;
						}
					...
					// If the term has a parent, unset the parent so we only assign the child
					if ( ! empty( $term_args['parent'] ) )
						$terms_to_set[$taxonomy] = array_diff( $terms_to_set[$taxonomy], array( $term_args['parent'] ) );
