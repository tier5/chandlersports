...
class WC_CSV_Product_Variation_Import extends WC_CSV_Product_Import {
	...
	function process_product( $post ) {
		...
		if ($merging) {
			...
			// Insert product
			$this->log->add( sprintf( __('> Inserting %s.', 'wc_csv_import'), esc_html($post['post_title']) ) );
		...
		if ( ! empty( $post['terms'] ) ) {
			...
			foreach ( $post['terms'] as $term_group ) {
				...
				foreach ( $terms as $term ) {
					...
					if ( ! $term || ! $taxonomy || ! taxonomy_exists( $taxonomy ) ) continue;
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							$this->log->add( sprintf( __( '> > Inserted term %s %s', 'wc_csv_import' ), esc_html($term_name), esc_html($taxonomy) ) );
							$term_id = $t['term_id'];
						} else {
						...
						$this->log->add( sprintf( __( '> > Found term %s %s', 'wc_csv_import' ), esc_html($term_name), esc_html($taxonomy) ) );
