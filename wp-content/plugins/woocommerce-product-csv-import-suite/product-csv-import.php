 ...
 **/
if (is_admin()) {
	$woo_plugin_updater_csv_importer = new WooThemes_Plugin_Updater( __FILE__ );
	$woo_plugin_updater_csv_importer->api_key = '3924989942c7ea4d62d794fb2571eb05';
	$woo_plugin_updater_csv_importer->init();
}
	...
	if (!class_exists('WC_Product_CSV_Import_Suite')) {
		...
		class WC_Product_CSV_Import_Suite {
			...
			function product_exporter( $post_type = 'product' ) {
				...
				foreach ( $products as $product ) {
					...
					foreach ( $meta_data as $meta => $value ) {
						...
						if ( is_array( $meta_value ) ) {
							if ( ! isset( $meta_value[0] ) || is_array( $meta_value[0] ) ) continue; // Can't handle multidimensional arrays
				...
				$found_product_meta = array_diff( array_unique( $found_product_meta ), array_keys( $csv_columns ) );
				...
				foreach ( $products as $product ) {
					...
					foreach ( $csv_columns as $column => $value ) {
						if ( ! $export_columns || in_array( $column, $export_columns ) ) {
							...
							} elseif ( isset( $product->meta->$column ) ) {
