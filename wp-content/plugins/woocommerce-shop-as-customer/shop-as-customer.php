 ...
 * Define Constants
 */
define( 'WC_SHOP_AS_CUSTOMER_VERSION', '2.06' );
define( 'WC_SHOP_AS_CUSTOMER_REQUIRED_WOOCOMMERCE_VERSION', 2.3 );
...
class WC_Shop_As_Customer {
	...
	*/
	...
	public function enqueue_scripts() {
		...
		// Set suffix to use with enqued scripts.
		$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
		
		// We must register Select2 & Enhanced-Select & Admin Styles from WooCommerce in order to search the customers on the front end.
		wp_register_script( 'select2', WC()->plugin_url() . '/assets/js/select2/select2' . $suffix . '.js', array( 'jquery' ), WC_SHOP_AS_CUSTOMER_VERSION );
		...
		if ( ! wp_style_is( 'woocommerce_admin_styles', 'enqueued' ) ) {
			// Enqueue our own Select2 CSS if WooCommerce admin.css isn't enqueued on
			// this page (dictated by their `wc_get_screen_ids()` helper function).
			wp_enqueue_style(
				'woocommerce-shop-as-customer-select-2',
				WC_SHOP_AS_CUSTOMER_URI . '/assets/css/shop-as-customer-select-2.css',
				array(),
		...
		if ( is_admin() && isset( $screen->id ) && 'settings_page_shop_as_customer_settings' == $screen->id ) {
	...
	function switch_to_customer( $switch_to_user_id, $remember = false, $switch_direction = 'switch_to_customer' ) {
		...
		*/
		...
		do_action( 'wp_login', $switch_to_user->user_login );
