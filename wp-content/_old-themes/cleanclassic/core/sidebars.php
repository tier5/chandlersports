...
// tokens
  ...
  $sidebars = array(
   ...
   'default' => array(
    ...
    'top' => array(
      'name' => __('Top Widget Area','kubrick'),
      'id' => 'top-widget-area',
      'description' => __("This sidebar is displayed above the main content.")
    ),
    
    'bottom' => array(
      'name' => __('Bottom Widget Area','kubrick'),
      'id' => 'bottom-widget-area',
      'description' => __("This sidebar is displayed below the main content.")
    ),
    
    'footer' => array(
      'name' => __('First Footer Widget Area','kubrick'),
      'id' => 'first-footer-widget-area',
      'description' => __("The first footer widget area. You can add a text widget for custom footer text.") 
    ),
...
function art_get_sidebar($name){
    ...
    if (!art_dynamic_sidebar($name))  {
        $sidebar = (isset($art_config['sidebars'][$name]) ? $art_config['sidebars'][$name] : '');
        if ($sidebar != '') {
            ...
            for($i = 0; $i < count($blocks); $i++){
                ...
                switch($block) {
                    case 'search':
                        echo art_get_block( __('Search', 'kubrick'), art_get_search());
                    break;
                    case 'archive': 
                        ob_start();
                        wp_get_archives('type=monthly&title_li=');// 2.6 not supported echo=0
                        echo art_get_block( __('Archives', 'kubrick'), '<ul>'.ob_get_clean().'</ul>');
                    break;
                    case 'categories':
                        echo art_get_block( __('Categories', 'kubrick'), '<ul>'.wp_list_categories('show_count=1&title_li=&echo=0').'</ul>');
                    break;
                    case 'blogroll':
                        echo art_get_block( __('Bookmarks', 'kubrick'), '<ul>'.wp_list_bookmarks('title_li=&categorize=0&echo=0').'</ul>');
                    break;
                    case 'vmenu':
                        echo art_get_vmenu();
                    break; 
